...
export default function AboutPage() {
  return (
    <div className="pb-16">
      {/* Hero */}
      <section className="bg-brand-primary-dark py-16 text-text-on-primary sm:py-20">
        <div className="page-container max-w-3xl text-center">
          <h1 className="text-3xl font-semibold tracking-tight text-text-on-primary sm:text-4xl">
            About {STORE.name}
          </h1>
          <p className="mt-4 text-lg text-text-on-primary/85">
      ...
      <section className="page-container py-16">
        <div className="mx-auto max-w-3xl">
          <SectionHeading align="left" title="Furniture you can trust, from people you know" />
          <RevealOnScroll className="mt-6 space-y-4 leading-7 text-text-secondary">
            <p>
      ...
      <section className="bg-brand-cream-dark py-16">
        <div className="page-container">
          <SectionHeading title="The Pragati promise" />
          <StaggerGroup className="mt-10 grid gap-6 sm:grid-cols-2">
            {values.map((value) => (
              <StaggerItem key={value.title}>
                <div className="h-full rounded-xl border border-brand-border bg-brand-surface p-6">
                  <h3 className="text-lg font-semibold text-text-primary">{value.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-text-secondary">{value.text}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerGroup>
      ...
      <section className="page-container py-16 text-center">
        <h2 className="text-2xl font-semibold text-text-primary">Come see our furniture in person</h2>
        ...
        <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <Link
            href="/"
            className="btn-primary w-full rounded-lg bg-brand-primary px-6 py-3 text-sm font-semibold text-text-on-primary hover:bg-brand-primary-dark sm:w-auto"
          >
            Back to home
          </Link>
          <Link
            href="/contact"
            className="w-full rounded-lg border border-brand-primary px-6 py-3 text-sm font-semibold text-brand-primary transition-colors hover:bg-brand-primary hover:text-text-on-primary sm:w-auto"
          >
            Contact & directions
          </Link>
        </div>
